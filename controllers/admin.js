...
const getBlockedUsers = async (req,res)=>{
  ...
  if(!blockedUsers){
      return res.status(StatusCodes.NOT_FOUND).json({message:"No user is blocked"})
  }
...
const getUnblockedUsers = async (req,res)=>{
  ...
  if(!unblockedUsers){
      return res.status(StatusCodes.NOT_FOUND).json({message:"No user is unblocked"})
  }
...
const unblockUserLogin = async (req, res) => {
 ...
 const getUserID = async (req,res)=>{
  ...
  try{
    ...
    if(!user){
      return res.status(StatusCodes.NOT_FOUND).json({message:'User doesnt exist'})
    }
...
//need further details to test this API, have to know beforehand which actions are to be blocked
  ...
  const blockUserActions = async(req,res)=>{
    ...
    try {
      ...
      res.status(200).json({ message: `User action ${actionType} blocked`, user });
    } catch (error) {
  ...
  const approveUserActions = async(req,res)=>{
    ...
    try {
      ...
      res.status(200).json({ message: `User action ${actionType} approved`, user });
    } catch (error) {
